# Internal Plug-in ETAS approximation used only for MCMC initialisation.
...
optional_numeric_env <- function(name) {
  value <- Sys.getenv(name, unset = "")
  if (!nzchar(value) || toupper(value) == "NA") return(NA_real_)
  parsed <- suppressWarnings(as.numeric(value))
  if (length(parsed) != 1L || !is.finite(parsed)) {
    stop(name, " must be a finite number when supplied.")
  }
  parsed
}
...
config <- list(
  ...
  oracle_history_path = Sys.getenv("ORACLE_HISTORY_PATH", unset = ""),
  oracle_detection_path = Sys.getenv("ORACLE_DETECTION_PATH", unset = ""),
  ...
  tvc_settings = list(
    detection_model = Sys.getenv("DETECTION_MODEL", unset = "exponential_recovery"),
    M_trigger = as.numeric(Sys.getenv("M_TRIGGER", unset = "5.5")),
    gamma = as.numeric(Sys.getenv("MCT_GAMMA", unset = "0.8")),
    tau = as.numeric(Sys.getenv("MCT_TAU", unset = "1")),
    mct_mode = Sys.getenv("MCT_MODE", unset = "sum"),
    b_GR = as.numeric(Sys.getenv("B_GR", unset = "1")),
    G = optional_numeric_env("LOGLINEAR_G"),
    H = optional_numeric_env("LOGLINEAR_H"),
    mainshock_time = optional_numeric_env("MAINSHOCK_TIME"),
    mainshock_magnitude = optional_numeric_env("MAINSHOCK_MAGNITUDE")
  )
)

fit_out <- fit_inlabru_tvc(config)
