...
#   MCMC_MODELS=naive,plugin,oracle,complete
...
valid_models <- c("naive", "plugin", "oracle", "complete")
if (!all(models %in% valid_models)) {
  stop("MCMC_MODELS must contain only: ", paste(valid_models, collapse = ", "))
}

model_seed_offset <- c(naive = 0L, plugin = 100L, oracle = 200L, complete = 300L)
...
for (id in ids) {
  for (model in models) {
    catalogue_type <- if (model == "complete") "complete" else "observed"
    input_path <- file.path(
      cfg$data_dir, sprintf("%s_%03d.rds", catalogue_type, id)
    )
    ...
    initialisation_model <- if (model %in% c("plugin", "oracle")) {
      "plugin"
    } else {
      "naive"
    }
    initialisation_id <- id
    inla_path <- file.path(
      cfg$results_dir, "initialisation",
      sprintf("%s_%03d.rds", initialisation_model, initialisation_id)
    )
    if (!file.exists(inla_path) && model %in% c("oracle", "complete")) {
      initialisation_id <- as.integer(
        Sys.getenv("BENCHMARK_PILOT_ID", unset = "1")
      )
      inla_path <- file.path(
        cfg$results_dir, "initialisation",
        sprintf("%s_%03d.rds", initialisation_model, initialisation_id)
    ...
    initialisation_input_path <- file.path(
      cfg$data_dir, sprintf("observed_%03d.rds", initialisation_id)
    )
    initialisation_md5 <- unname(tools::md5sum(initialisation_input_path))
    stopifnot(
      identical(inla$input_md5, initialisation_md5),
    ...
    if (model == "plugin") {
      ...
      gh <- c(G = cfg$incompleteness$G, H = cfg$incompleteness$H)
    }
    if (fit_is_reusable(output_path, input_md5, model, settings)) {
      cat("Reusing MCMC fit:", model, id, "\n")
      next
    }
    if (model %in% c("naive", "plugin") &&
        import_legacy_fit(output_path, input_md5, id, model, gh)) {
    ...
    log_posterior <- if (model %in% c("naive", "complete")) {
    ...
    run_one <- function(chain) {
      result <- run_exact_etas_chain(
        ...
        seed = 730000L + 1000L * id + model_seed_offset[[model]] + chain
      )
      values <- t(apply(
    ...
    saveRDS(
      list(
        ...
        settings = settings,
        initialisation_model = initialisation_model,
        initialisation_sim_id = initialisation_id,
        seed_rule = paste0(
          "730000 + 1000 * sim_id + model offset ",
          model_seed_offset[[model]], " + chain"
        ),
        imported_from = NULL
      ),
      output_path
