...
required_files <- c(
  "writing/first_draft_feedback.tex",
  "writing/refs.bib",
...
assert_close(get_forecast("naive", "log_score_per_event"), 2.4036,
             label = "Naive temporal LPD")
assert_close(get_forecast("plugin", "log_score_per_event"), 2.4293,
             label = "Plug-in temporal LPD")
assert_close(forecast_mc$information_gain_per_event, 0.0257,
             label = "information gain")
assert_close(forecast_mc$mc_q025, 0.0241, label = "information-gain lower bound")
assert_close(forecast_mc$mc_q975, 0.0276, label = "information-gain upper bound")
for (model_name in c("naive", "plugin")) {
  targets <- if (model_name == "naive") {
    c(count_q025 = 71, count_median = 174, count_q975 = 740.6,
      overflow_rate = 0.005, probability_supercritical = 0.01884375)
  } else {
    c(count_q025 = 146, count_median = 257.5, count_q975 = 1259.175,
      overflow_rate = 0.017, probability_supercritical = 0.83803125)
  }
...
assert_close(max(ridgecrest_diagnostics$rhat), 1.002, tolerance = 5e-4,
             label = "maximum Ridgecrest R-hat")
assert_close(min(ridgecrest_diagnostics$effective_size), 2578, tolerance = 0.6,
             label = "minimum Ridgecrest ESS")

# Every citation key and cross-reference used by the manuscript must resolve.
tex <- paste(readLines("writing/first_draft_feedback.tex", warn = FALSE), collapse = "\n")
bib <- paste(readLines("writing/refs.bib", warn = FALSE), collapse = "\n")
...
bib_keys <- regmatches(bib, gregexpr("(?<=@\\w{1,20}\\{)[^,]+", bib, perl = TRUE))[[1L]]
...
r_files <- c(Sys.glob("R/*.R"), Sys.glob("config/*.R"),
             Sys.glob("scripts/submission/*.R"),
             Sys.glob("scripts/submission/internal/*.R"))
...
cat("Checked manuscript claims, citations, references, figures, and R syntax.\n")
